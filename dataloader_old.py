...
class CDSRDataset(Dataset):
    ...
    def get_gt_spe(self, seq_x, seq_raw):
        """ make ground truths for domain-specific sequences """
        seq = np.array(seq_raw)
        gt_a_raw = np.append(seq[seq <= self.n_item_a][1:], -1)  # pad the last interaction of a/b domain with gt -1
        gt_b_raw = np.append(seq[seq > self.n_item_a][1:], -1)

        gt_a, gt_b = np.array([], dtype=np.int32), np.array([], dtype=np.int32)

        for i, idx in enumerate(seq_x):
            if idx <= self.n_item_a:
                gt_a = np.append(gt_a, gt_a_raw[0])
                gt_a_raw = gt_a_raw[1:]

                gt_b = np.append(gt_b, 0)

            else:
                gt_a = np.append(gt_a, 0)

                gt_b = np.append(gt_b, gt_b_raw[0])
                gt_b_raw = gt_b_raw[1:]

        gt_a[gt_a < 0] = 0  # remove the gt -1
        gt_b[gt_b < 0] = 0
        return gt_a, gt_b
    ...
    def process_train(self, idx_u, seq_raw):
        """ change the training partition and alignment according to C2DSR and DREAM """
        idx_u = np.array(idx_u)
        seq_x = seq_raw[:-3]

        seq_a = seq_x.copy()
        seq_a[seq_a > self.n_item_a] = 0

        seq_b = seq_x.copy()
        seq_b[(0 < seq_b) & (seq_b <= self.n_item_a)] = 0

        gt_a, gt_b = self.get_gt_spe(seq_x, seq_raw[:-2])

        seq_x, seq_a, seq_b, gt_a, gt_b = \
            (self.trim_seq(s) for s in (seq_x, seq_a, seq_b, gt_a, gt_b))

        gt = gt_a + gt_b
        mask_gt_a = (gt_a != 0).astype(int)
        mask_gt_b = (gt_b != 0).astype(int)
    ...
    def process_valid(self, idx_u, seq_raw):
        """ since evaluation only requires last available token, evaluation partition does not need to change"""
        idx_u = np.array(idx_u)
        seq_x = seq_raw[:-2]
