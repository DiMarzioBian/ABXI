...
def process_train(args, seq_raw):
    """ process training sequences """
    seq_x, gt = seq_raw[:-1], seq_raw[1:]

    seq_a, seq_b = get_spe_seq(args.n_item_a, seq_x, gt)

    return *(trim_seq(x, args.len_trim) for x in (seq_x, seq_a, seq_b, gt)), seq_raw


def process_evaluate(args, seq_raw):
    """ process evaluation sequences """
    seq, gt = seq_raw[:-1], seq_raw[-1:]

    return trim_seq(seq, args.len_trim), gt, seq_raw
...
def get_dataset(args):
    ...
    if args.raw:
        ...
        for seq in tqdm(data_seq, desc='processing', leave=False):
            data_tr.append(process_train(args, seq[:-2]))
            data_val.append(process_evaluate(args, seq[:-1]))
            data_te.append(process_evaluate(args, seq))
